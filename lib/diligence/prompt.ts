...
import type { SystemBlock } from "@/lib/llm/types";
...
- Do NOT use em-dashes (—), en-dashes (–), or semicolons (;) anywhere in the text values; use a regular hyphen (-), commas (,), or rephrase/split into multiple sentences instead.

## Fields
${fieldGuide()}`;

/**
 * The shared, cacheable prefix every stage sends: persona + playbook (stable
 * across the whole analysis, but too small on its own to be worth a
 * breakpoint) followed by the deck text, which carries the cache breakpoint.
 * The Anthropic cache is a prefix match, so marking only the last block of a
 * stable span is enough to cache everything before it too — every later
 * stage of this same analysis that sends this exact prefix reads it from
 * cache instead of paying full price again.
 */
function baseBlocks(playbook: string, deckText: string): SystemBlock[] {
  return [
...
/**
 * Research findings + sources, appended after the deck block with its own
 * cache breakpoint — the complete/scorecard/feedback stages all send this
 * same block, so after the first of them writes it, the rest read it from
 * cache instead of re-paying for the whole research write-up.
 */
function findingsBlock(research: ResearchResult): SystemBlock {
  const sourceList =
...
export function buildDeckExtractSystemPrompt(playbook: string, deckText: string): SystemBlock[] {
  return [
    ...baseBlocks(playbook, deckText),
    {
      text: `You are given the extracted text of a startup's pitch deck above. Fill in every field of the due-diligence form that you can find IN THE DECK. Use source "deck" for everything here. Skip fields the deck doesn't cover (especially "company.source" and "company.personalNote", which come from the investor's own meeting and are almost never in a deck). Do NOT score the scorecard yet — skip all scorecard.* fields in this pass.
...
export function buildDeckExtractUserPrompt(): string {
  return "Emit the deck-derived fields as NDJSON now.";
}

// ───────────────────────── Pass 2: web research ─────────────────────────

export function buildResearchSystemPrompt(playbook: string, deckText: string): SystemBlock[] {
  return [
    ...baseBlocks(playbook, deckText),
    {
      text: `You are given a startup's pitch deck above. You will also be told the company name and a list of FACTS STILL MISSING after reading the deck. Research the company on the web to support a due-diligence form.
...
export function buildResearchUserPrompt(companyName: string, gaps: string[]): string {
...
export function buildCompleteSystemPrompt(
  ...
  return [
    ...
    {
      text: `You are given (a) a startup's pitch deck and (b) research notes already gathered, both above. Produce the COMPLETE due-diligence form: fill every field you can, preferring the most accurate value (use the deck for claims, the research to verify/augment/correct).
...
export function buildCompleteUserPrompt(): string {
  return "Now emit the complete form as NDJSON.";
}
 ...
 */
export function buildScorecardSystemPrompt(
  playbook: string,
  deckText: string,
  research: ResearchResult,
): SystemBlock[] {
  return [
    ...
    findingsBlock(research),
    {
      text: `Score this startup on William's seven-factor scorecard, using the deck and research notes above. This is a judgment call — you always have enough to give your best estimate, so NEVER leave a value out.
...
export function buildScorecardUserPrompt(): string {
  return "Now output the scorecard JSON object.";
}
 ...
 */
export function buildDeckFeedbackSystemPrompt(
  playbook: string,
  deckText: string,
  research: ResearchResult,
): SystemBlock[] {
  return [
    ...
    findingsBlock(research),
    {
      text: `You are given a startup's pitch deck plus research notes gathered about the company, both above. Write a BALANCED critique of the PITCH DECK ITSELF — not the company's prospects — as if advising the founder on what to fix before sending it to investors.
...
- Do NOT use em-dashes (—), en-dashes (–), or semicolons (;) in any of the fields (title, detail, category); use a regular hyphen (-) or commas (,) instead.
- Output nothing except the JSON lines.`,
    },
  ];
}

export function buildDeckFeedbackUserPrompt(): string {
  return "Now output the deck feedback as NDJSON.";
}
